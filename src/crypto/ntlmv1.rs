use crate::{
    ...
    messages::{
        ...
        structures::{
            ...
            LmChallenge, Lmv1Challenge, NtChallenge, Ntv1Challenge, ServerChallenge,
            SessionBaseKey,
        },
    },
};
...
pub fn compute_response(
...
) -> (LmChallenge, NtChallenge, SessionBaseKey) {
    ...
    if flags.has_flag(NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY) {
        ...
        (lm_response.into(), nt_response.into(), session_base_key)
    } else {
        let nt_response = Ntv1Challenge {
...
pub fn encrypt_random_session_key(
    ...
    {
        eprintln!("CASE #1");
        let exported_session_key = if cfg!(test) {
            eprintln!("Using not so random exported_session_key");
            ExportedSessionKey::from(tests::RANDOM_SESSION_KEY)
        } else {
...
mod tests {
    ...
    fn without_extented_session_security() {
        let nt_response: NtChallenge = Ntv1Challenge {
            response: [
        ...
        .into();
        let lm_response: LmChallenge = Lmv1Challenge {
            response: [
    ...
    fn encrypted_session_key() {
        ...
        pretty_assertions::assert_eq!(
            ...
            encrypt_random_session_key(&flags, &key_exchange_key).1
        );
    }
}
